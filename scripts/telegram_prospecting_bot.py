...
PRODUCTS = {
    ...
    "cartvertising": {
        ...
        "packages": {
            "20_single": {"name": "20% Front OR Directory", "base_6month": 2995},
            "40_both": {"name": "40% (20% Front + 20% Directory)", "base_6month": 4795},
            "60_both": {"name": "60% (40% Front + 20% Directory)", "base_6month": 5995},
            "80_both": {"name": "80% (40% Front + 40% Directory)", "base_6month": 7395},
            "100_both": {"name": "100% (60% Front + 40% Directory)", "base_6month": 8795},
            "200_both": {"name": "200% (100% Both Sides)", "base_6month": 12995},
            "header_50": {"name": "Header 50% (Every Other Cart)", "base_6month": 2995},
            "header_100": {"name": "Header 100% (Every Cart)", "base_6month": 4795},
        }
    },
    "digitalboost": {
...
def add_to_cart(rep_id: str, product_type: str, tier: str = None, store_num: str = None, 
    ...
    # === CARTVERTISING ===
        ...
        # === CARTVERTISING: PACKAGE-BASED PRICING ===
        # Extract package_id from kwargs (e.g., '20_single', '40_both', 'header_50', etc.)
        package_id = kwargs.get('package_id')
        if not package_id:
            return None
        
        # Get package from PRODUCTS config
        packages = PRODUCTS["cartvertising"].get("packages", {})
        if package_id not in packages:
            return None
        
        package_info = packages[package_id]
        package_name = package_info.get("name", "")
        base_6month = package_info.get("base_6month", 0)
        
        # === PAYMENT PLAN CONVERSION ===
        # 6-month base price is defined in packages
        # Convert to other payment plans using the conversion formula:
        # 6-month = annual ÷ 2  (so annual = 6-month × 2)
        annual_price = base_6month * 2
        
        if payment_plan == "monthly":
            price = annual_price / 12
        elif payment_plan == "3month":
            price = (annual_price / 2) / 2  # 6-month ÷ 2
        elif payment_plan == "6month":
            price = base_6month
        elif payment_plan == "pif":
            price = annual_price * 0.95  # 5% discount
        else:
            price = base_6month  # Default to 6-month
        
        # === COST METRICS ===
        cost_per_day = price / (365 if payment_plan != "monthly" else 30)
        cpm = 0  # Cartvertising doesn't use CPM (no fixed impressions)
        
        # === BUILD CART ITEM ===
        cart_item = {
            "product_type": "cartvertising",
            "product_name": f"Cartvertising — {package_name}",
            "package_id": package_id,
            "package_name": package_name,
            "payment_plan": payment_plan,
            "price": price,
            "base_6month": base_6month,
...
async def show_product_child_seat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show Child Seat (Cartvertising) package selection with all 8 package options."""
    query = update.callback_query
    await query.answer()
    
    text = (
        "🛒 *CARTVERTISING PACKAGES*\n\n"
        "_All packages: 360,000 annual impressions_\n\n"
        "📊 *Select a package:*"
    )
    
    # Create buttons for all 8 packages with payment plan selection
    buttons = [
        [InlineKeyboardButton("🎬 Presentation", url="https://docs.google.com/presentation/d/1xwIF4CaTp07AKunGaJysCSIGqN7VCdbL4fgOH3XEpl4/edit?usp=sharing")],
        [InlineKeyboardButton("📹 Explainer Video", url="https://www.youtube.com/watch?v=PduxHWy8sMc")],
        # Package selection
        [InlineKeyboardButton("20% Front/Directory — $2,995", callback_data="cart_pkg_20_single")],
        [InlineKeyboardButton("40% (20F+20D) — $4,795", callback_data="cart_pkg_40_both")],
        [InlineKeyboardButton("60% (40F+20D) — $5,995", callback_data="cart_pkg_60_both")],
        [InlineKeyboardButton("80% (40F+40D) — $7,395", callback_data="cart_pkg_80_both")],
        [InlineKeyboardButton("100% (60F+40D) — $8,795", callback_data="cart_pkg_100_both")],
        [InlineKeyboardButton("200% (100% Both) — $12,995", callback_data="cart_pkg_200_both")],
        [InlineKeyboardButton("Header 50% (Every Other) — $2,995", callback_data="cart_pkg_header_50")],
        [InlineKeyboardButton("Header 100% (Every Cart) — $4,795", callback_data="cart_pkg_header_100")],
...
async def show_cartvertising_payment_plan(update: Update, context: ContextTypes.DEFAULT_TYPE, package_id: str):
    """Show payment plan selection for a Cartvertising package."""
    query = update.callback_query
    await query.answer()
    ...
    text = (
        f"🛒 *{package_name}*\n\n"
        f"📊 *Pricing:*\n"
        f"• Base (6-month): ${base_6month:,.2f}\n"
        f"• Annual: ${annual_price:,.2f}\n"
        f"• CPM (360K impressions): ${(annual_price / 360000 * 1000):.2f}\n\n"
        f"📅 *Select payment plan:*"
    )
    
    buttons = [
        [InlineKeyboardButton("📅 Monthly (÷12)", callback_data=f"cart_add_cartvertising_pkg_{package_id}_monthly")],
        [InlineKeyboardButton("📅 3-Month (÷2 of 6m)", callback_data=f"cart_add_cartvertising_pkg_{package_id}_3month")],
        [InlineKeyboardButton("📅 6-Month (Base)", callback_data=f"cart_add_cartvertising_pkg_{package_id}_6month")],
        [InlineKeyboardButton("📅 Paid in Full (-5%)", callback_data=f"cart_add_cartvertising_pkg_{package_id}_pif")],
...
async def show_cart_standard_pricing(update: Update, context: ContextTypes.DEFAULT_TYPE, product: str = "child_seat"):
    """[DEPRECATED - kept for compatibility] Show Standard pricing tiers."""
    query = update.callback_query
    await query.answer()
...
"""
        ...
        # ROI Parameter Adjustments (text input)
        ...
        elif data.startswith("cart_pkg_"):
            # Package selection: cart_pkg_20_single, cart_pkg_40_both, etc.
            package_id = data.replace("cart_pkg_", "")
            await show_cartvertising_payment_plan(update, context, package_id)
        
        elif data.startswith("cart_add_cartvertising_pkg_"):
            # Add package to cart: cart_add_cartvertising_pkg_{package_id}_{payment_plan}
            await query.answer("✅ Added to cart!", show_alert=False)
            
            # Parse: cart_add_cartvertising_pkg_20_single_monthly
            parts = data.replace("cart_add_cartvertising_pkg_", "").rsplit("_", 1)
            package_id = "_".join(parts[:-1])  # Rejoin in case package_id has underscores
            payment_plan = parts[-1]  # 'monthly', '3month', '6month', 'pif'
            ...
            if price:
                ...
                base_6month = package_info.get("base_6month", 0)
                annual_price = base_6month * 2
                plan_label = "Monthly" if payment_plan == "monthly" else (
                    "3-Month" if payment_plan == "3month" else (
                        "6-Month" if payment_plan == "6month" else "Paid in Full"
                    )
                )
                ...
                await query.edit_message_text(
                    f"✅ *Added to Cart!*\n\n"
                    f"🛒 {package_name}\n"
                    f"📊 360,000 annual impressions\n"
                    f"💳 {plan_label} plan\n"
                    f"💰 ${price:,.2f} ({plan_label})\n"
                    f"📈 Annual: ${annual_price:,.2f}\n\n"
            ...
            # Legacy support for old standalone/bundled format
            await query.answer("✅ Added to cart!", show_alert=False)
            ...
            rep_id = get_rep_id(update)
            # Map to a default package: use 20_single for standalone, 40_both for bundled
            package_id = "40_both" if bundle_type == "bundled" else "20_single"
            price = add_to_cart(
                rep_id=rep_id,
                product_type="cartvertising",
                package_id=package_id,
            ...
            for idx, item in enumerate(cart_items):
                ...
                if product_type == "register_tape":
                    ...
                    text += f"📊 Package: {item.get('package_name', item.get('bundle_type', 'Standard'))}\n"
                    text += f"💳 Plan: {item.get('payment_plan', 'monthly').replace('_', '-').title()}\n"
                elif product_type == "digitalboost":
...
"""
            ...
            for idx, item in enumerate(cart_items, 1):
                ...
                if product_type == "register_tape":
                    ...
                    proposal_text += f"   Package: {item.get('package_name', item.get('bundle_type', 'Standard'))}\n"
                    proposal_text += f"   Payment Plan: {item.get('payment_plan', 'monthly').replace('_', '-').title()}\n"
                elif product_type == "digitalboost":
            ...
            try:
                ...
                for item in cart_items:
                    ...
                    if product_type == "register_tape":
                        ...
                        package_name = item.get('package_name', item.get('bundle_type', 'Standard'))
                        plan = item.get('payment_plan', 'monthly').replace('_', '-').title()
                        details = f"{package_name}\n{plan} Plan"
                    elif product_type == "digitalboost":
                ...
                if rt_items:
                    ...
                    for item in rt_items:
                        ...
                        # Metrics table (only for products with impressions data)
                        if item.get('annual_impressions'):
                            metrics_data = [
                                ["Metric", "Value"],
