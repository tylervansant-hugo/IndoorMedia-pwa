<script>
  ...
  async function exportPricingSheet(store, pricing, adType, lang) {
    ...
    const isPromoPdf = pricingMode === 'summer_promo';
    ...
    const plans = isPromoPdf ? [
      ...
      { name: `Paid in 3 (${pricing.threeMonthPayments || 3} installments)`, payment: `$${pricing.threeMonth} x ${pricing.threeMonthPayments || 3}`, total: pricing.threeMonthTotal, daily: (parseFloat(pricing.threeMonthTotal) / 365).toFixed(2) },
      { name: `Paid in 6 (${pricing.sixMonthPayments || 6} installments)`, payment: `$${pricing.sixMonth} x ${pricing.sixMonthPayments || 6}`, total: pricing.sixMonthTotal, daily: (parseFloat(pricing.sixMonthTotal) / 365).toFixed(2) },
  ...
  function exportPricingSheetHTML(store, pricing, adType, lang) {
    ...
    const isPromoHTML = pricingMode === 'summer_promo';
...
<div class="footer">${t.poweredBy}</div>
  ...
  const ROI_TRANSLATIONS = {
    en: { title: 'ROI Analysis Report', inputs: 'Inputs', investment: 'Investment', avgSpend: 'Avg Customer Spend', newCust: 'New Customers / Month', visits: 'Visits / Year', cogs: 'Cost of Goods Sold', revenue: 'Revenue Analysis', grossAnnual: 'Gross Annual Revenue', cogsLabel: 'Cost of Goods Sold', netAnnual: 'Net Annual Revenue', results: 'Results', roi: 'Return on Investment', netProfit: 'Net Profit', generated: 'Generated', registerTape: 'Register Tape Advertising', printSave: 'Print / Save as PDF' },
    es: { title: 'Informe de Análisis ROI', inputs: 'Datos', investment: 'Inversión', avgSpend: 'Gasto Promedio del Cliente', newCust: 'Nuevos Clientes / Mes', visits: 'Visitas / Año', cogs: 'Costo de Bienes Vendidos', revenue: 'Análisis de Ingresos', grossAnnual: 'Ingresos Brutos Anuales', cogsLabel: 'Costo de Bienes Vendidos', netAnnual: 'Ingresos Netos Anuales', results: 'Resultados', roi: 'Retorno de Inversión', netProfit: 'Ganancia Neta', generated: 'Generado', registerTape: 'Publicidad en Cinta de Caja', printSave: 'Imprimir / Guardar como PDF' },
    zh: { title: 'ROI分析报告', inputs: '输入数据', investment: '投资', avgSpend: '平均客单价', newCust: '每月新客户', visits: '每年回访次数', cogs: '商品成本', revenue: '收入分析', grossAnnual: '年毛收入', cogsLabel: '商品成本', netAnnual: '年净收入', results: '结果', roi: '投资回报率', netProfit: '净利润', generated: '生成于', registerTape: '收银台小票广告', printSave: '打印 / 保存为PDF' },
    vi: { title: 'Báo Cáo Phân Tích ROI', inputs: 'Dữ Liệu', investment: 'Đầu tư', avgSpend: 'Chi tiêu TB/khách', newCust: 'Khách mới / Tháng', visits: 'Lượt ghé / Năm', cogs: 'Giá vốn hàng bán', revenue: 'Phân Tích Doanh Thu', grossAnnual: 'Tổng doanh thu năm', cogsLabel: 'Giá vốn hàng bán', netAnnual: 'Doanh thu ròng năm', results: 'Kết Quả', roi: 'Tỷ suất hoàn vốn', netProfit: 'Lợi nhuận ròng', generated: 'Tạo ngày', registerTape: 'Quảng cáo hóa đơn', printSave: 'In / Lưu PDF' },
    tl: { title: 'ROI Analysis Report', inputs: 'Mga Input', investment: 'Puhunan', avgSpend: 'Avg na Gastos ng Customer', newCust: 'Bagong Customer / Buwan', visits: 'Bisita / Taon', cogs: 'Halaga ng Benta', revenue: 'Pagsusuri ng Kita', grossAnnual: 'Taunang Gross na Kita', cogsLabel: 'Halaga ng Benta', netAnnual: 'Taunang Net na Kita', results: 'Mga Resulta', roi: 'Return on Investment', netProfit: 'Net na Kita', generated: 'Ginawa', registerTape: 'Register Tape Advertising', printSave: 'I-print / I-save bilang PDF' },
    ko: { title: 'ROI 분석 보고서', inputs: '입력 데이터', investment: '투자', avgSpend: '평균 고객 지출', newCust: '월 신규 고객', visits: '연간 방문 횟수', cogs: '매출원가', revenue: '수익 분석', grossAnnual: '연간 총 수익', cogsLabel: '매출원가', netAnnual: '연간 순 수익', results: '결과', roi: '투자 수익률', netProfit: '순이익', generated: '생성일', registerTape: '영수증 광고', printSave: '인쇄 / PDF 저장' }
  };
  ...
  function exportROI(store, lang = 'en') {
    ...
    const roiCalc = sharedCalculateROI({
      investment,
      avgSpend: roiAvgSpend || 0,
      couponRedemptions: roiCouponRedemptions || 0,
    ...
    const netProfit = roiCalc.netProfit;
...
💵 Net Profit: $${roiCalc.netProfit.toLocaleString()}
...
— IndoorMedia Register Tape Advertising`;
  ...
  function setPricingMode(mode) {
    ...
    // Reset co-op/padding when switching modes
    coopUnlocked = {};
  }
  ...
  let storeQuarters = {}; // { storeName: 1|2|3|4 }
  ...
  function calcPricing(basePrice, isCoop = false, quarters = 4) {
    const prod = 125;
  ...
  function handleAddToCart(store, selectedAdType, plan) {
    ...
    const promoNoPad = isPromo && (coopUnlocked[store.StoreName] || false);
    const base = selectedAdType === 'double' ? store.DoubleAd : store.SingleAd;
    const currentQuarters = storeQuarters[store.StoreName] || 4;
    ...
    if (isPromo) {
      ...
      priceDisplay = {
        ...
        threeMonth: `$${pricing.threeMonth} × ${pricing.threeMonthPayments} = $${pricing.threeMonthTotal}`,
        sixMonth: `$${pricing.sixMonth} × ${pricing.sixMonthPayments} = $${pricing.sixMonthTotal}`,
        pif: `$${pricing.pif}`
      };
    } else {
      pricing = calcPricing(base, isCoop, currentQuarters);
      planNames = {
        monthly: `Monthly (${pricing.monthlyPayments} payments)`,
        threeMonth: '3-Month (10% off)',
        sixMonth: '6-Month (7.5% off)',
    ...
    {:else}
      <div class="store-grid">
        {#each filtered.filter(s => storeCycleFilter === 'all' || s.Cycle === storeCycleFilter) as store (store.StoreName)}
          ...
          {@const promoNoPad = isPromo && (coopUnlocked[store.StoreName] || false)}
          {@const currentQuarters = storeQuarters[store.StoreName] || 4}
          {@const stdPricing = calcPricing(basePrice, false, currentQuarters)}
          {@const coopPricing = calcPricing(basePrice, true, currentQuarters)}
          {@const promoPricing = calcPromoPricing(basePrice, currentQuarters, promoNoPad)}
          ...
          <div class="store-card" id="store-card-{store.StoreName}" class:expanded={isExpanded} class:coop-active={isCoop} class:claimed={isClaimed}>
            ...
            {#if isExpanded}
              {@const totalDays = currentQuarters * 3 * 30}
              <div class="expanded-pricing">
                ...
                <div class="pricing">
                  <div class="price-row">
                    ...
                    <span class="price-value">${pricing.monthly}/mo × {pricing.monthlyPayments} = ${pricing.monthlyTotal}</span>
                  </div>
                  <div class="price-row highlight">
                    <span class="price-label">Paid in Full</span>
                    <span class="price-value pif">${pricing.pif} ({isPromo ? '5' : '15'}% off)</span>
                  </div>
                </div>
                ...
                <div class="quarters-selector">
                  <span class="qs-label">Run Length:</span>
                  {#each [1, 2, 3, 4] as q}
                    <button class="qs-btn" class:active={currentQuarters === q} on:click={() => { storeQuarters[store.StoreName] = q; storeQuarters = storeQuarters; }}>
                      {q} {q === 1 ? 'Quarter' : 'Quarters'} ({q * 3}mo)
                    </button>
                  {/each}
                </div>
                ...
                <div class="plan-card" on:click={() => handleAddToCart(store, currentAdType, 'monthly')}>
                  <div class="plan-header">
                    ...
                    <span class="plan-badge">{pricing.monthlyPayments} payments</span>
                  </div>
                  <div class="plan-price">${pricing.monthly}<span class="per">/month</span></div>
                  <div class="plan-total">Total: ${pricing.monthlyTotal}</div>
                  <div class="plan-daily">That's just <strong>${(parseFloat(pricing.monthlyTotal) / totalDays).toFixed(2)}/day</strong></div>
                  {#if isPromo && currentQuarters >= 4}<div class="plan-promo-note">🎁 Free quarter included!</div>{/if}
                ...
                <div class="plan-card" on:click={() => handleAddToCart(store, currentAdType, 'threeMonth')}>
                  <div class="plan-header">
                    <span class="plan-name">📦 {isPromo ? 'Paid in 3' : '3-Month'}</span>
                    <span class="plan-badge{isPromo ? '' : ' save'}">{isPromo ? `${pricing.threeMonthPayments} installments` : 'Save 10%'}</span>
                  </div>
                  ...
                  <div class="plan-daily">That's just <strong>${(parseFloat(pricing.threeMonthTotal) / totalDays).toFixed(2)}/day</strong></div>
                  {#if isPromo && currentQuarters >= 4}<div class="plan-promo-note">🎁 Free quarter included!</div>{/if}
                ...
                <div class="plan-card" on:click={() => handleAddToCart(store, currentAdType, 'sixMonth')}>
                  <div class="plan-header">
                    <span class="plan-name">📦 {isPromo ? 'Paid in 6' : '6-Month'}</span>
                    <span class="plan-badge{isPromo ? '' : ' save'}">{isPromo ? `${pricing.sixMonthPayments} installments` : 'Save 7.5%'}</span>
                  </div>
                  ...
                  <div class="plan-daily">That's just <strong>${(parseFloat(pricing.sixMonthTotal) / totalDays).toFixed(2)}/day</strong></div>
                  {#if isPromo && currentQuarters >= 4}<div class="plan-promo-note">🎁 Free quarter included!</div>{/if}
                ...
                <div class="plan-card best" on:click={() => handleAddToCart(store, currentAdType, 'pif')}>
                  ...
                  <div class="plan-daily">That's just <strong>${(parseFloat(pricing.pif) / totalDays).toFixed(2)}/day</strong></div>
                  {#if isPromo && currentQuarters >= 4}<div class="plan-promo-note">🎁 Free quarter + 🏆 PIF bonus point!</div>{/if}
                ...
                {#if roiStore === store.StoreName}
                  <div class="roi-calculator">
                    ...
                    <div class="roi-coupon-box">
                      <div class="roi-coupon-header">🎟️ Coupon Redemptions</div>
                      <div class="roi-field">
                        <label>Total Redemptions per Month</label>
                        <input type="number" bind:value={roiCouponRedemptions} placeholder="40" min="0" />
                      </div>
                      <div class="roi-field">
                        <label>…of those, New Customers per Month</label>
                        <input type="number" bind:value={roiNewCustomers} placeholder="10" min="0" max={roiCouponRedemptions} />
                      </div>
                      <div class="roi-coupon-split">
                        <span><strong>{roiNewCustomers || 0}</strong> new</span>
                        <span><strong>{Math.max(0, (roiCouponRedemptions||0) - (roiNewCustomers||0))}</strong> returning / loyal</span>
                      </div>
                    </div>
                    
                    <div class="roi-field">
                      <label>Visits per Year (informational — already in compounding)</label>
...
<style>
  .quarters-selector { display: flex; align-items: center; gap: 6px; margin: 12px 0; flex-wrap: wrap; }
  .qs-label { font-size: 12px; font-weight: 700; color: var(--text-secondary); }
  .qs-btn { padding: 6px 12px; border: 1px solid var(--border-color); border-radius: 8px; background: var(--card-bg); font-size: 12px; font-weight: 600; cursor: pointer; color: var(--text-primary); }
  .qs-btn.active { background: #CC0000; color: white; border-color: #CC0000; }
  .search-container {
    max-width: 1200px;
    margin: 0 auto;
  ...
  .roi-coupon-box .roi-field:last-of-type { margin-bottom: 6px; }
  .roi-coupon-split {
    display: flex;
  ...
  .roi-coupon-split strong { color: #1a1a1a; }
